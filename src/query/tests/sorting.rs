...
fn single_asc() {
    ...
    assert_eq!(sorting[0].type_(), &bag.map().get("peoples").unwrap());
}
...
fn multiple_mixed() {
    ...
    assert_eq!(sorting[1].type_(), &bag.map().get("peoples").unwrap());
}
...
fn empty() {
    ...
    let err: CibouletteError = builder
        ...
        .expect_err("not to build correctly");
    assert_eq!(
        matches!(err, CibouletteError::UnknownField(type_, field) if type_.as_str() == "peoples" && field.as_str() == ""),
        true
    );
}
